...
# -*- coding: utf-8 -*-
"""
Classes to support oscilloscope over GPIB.
...
class DL1540:
    ...
    def screen_capture(self):
        ...
        image_data, num_lenbytes, num_databytes = self.handle_binary_response(data)
    ...
    def read_trace(self, channel):
        # Set some parameters
        self.write("COMM:HEAD 0")
        self.write("WAVEFORM:DATASELECT ACQDATA;FORMAT WORD;BYTEORDER LSBFIRST")
        ...
        data_per_block = 5000
        numblocks = int(data_length/data_per_block) + int(data_length%data_per_block > 0)
        this_ch_data = []
        for i in xrange(0,numblocks):
            ...
            if DEBUG:
            	print "Getting data from " + str(startblock)+' to '+str(endblock)
            self.write('WAV:START '+str(startblock)+';END '+str(endblock)+';SEND?')
            data = self.read(12000)
            # The scope first outputs the number of bytes of the length string
            trace_data, num_lenbytes, num_databytes = self.handle_binary_response(data)
            numeric_data = struct.unpack("<"+str(num_databytes/2)+"h",trace_data)
            numeric_data = list(numeric_data)
            this_ch_data += map(lambda x: float(x)*vdiv/3200+offset,numeric_data)
